...
export function buildRunbook(newSecret, now = new Date()) {
  ...
  const lines = [
    ...
    ``,
    `     pnpm --dir worker exec wrangler secret put RELAY_HMAC_SECRET_PREVIOUS`,
    ...
    `     # paste the NEW value shown above when prompted`,
    ``,
    ...
    ``,
    `     pnpm --dir worker exec wrangler secret delete RELAY_HMAC_SECRET_PREVIOUS`,
    ``,
